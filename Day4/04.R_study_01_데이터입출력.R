...
result
write.table(result, 'bmi2.txt', row.names = F, quote = T)
...
write.xlsx(result, 'bmi_result.xlsx', row.Names = F)
bmi_result.xlsx <- read_excel('./bmi_result.xlsx') 
bmi_result.xlsx
...
result <- subset(carprise.new, Type == input.type & MPG.highway >= input.city )
print(result)
