...
hist(iris$Sepal.Length, main = '붓꽃 꽃받침길이', 
     col = 'orange') # 히스토그램
...
# 2700 행 (C, M, N, S, SA)취합 하나의 테이블로 생성 
...
hist(tips.new$perHead, main = '1인당 주문액', breaks = 5, 
     xlab = '단위:$', ylab = '빈도수', las=2, col = color.5)
hist(lunch.perHead, main = '1인당 주문액(런치)', breaks = 4, 
     xlab = '단위:$', ylab = '빈도수', las=2, col = color.5)
hist(dinner.perHead, main = '1인당 주문액(디너)', breaks = 4, 
     xlab = '단위:$', ylab = '빈도수', las=2, col = color.5)
par(mfrow = c(1,1))
...
res.lunch <- hist(tips.new[tips.new$time == 'Lunch', 'percent'],
                  main = '주문액 대비 팁의 비율(런치)',
                  xlab = 'percent(%)', breaks = 5)
res.dinner <- hist(tips.new[tips.new$time == 'Dinner', 'percent'],
                   main = '주문액 대비 팁의 비율(디너)',
                   xlab = 'percent(%)', breaks = 5)
par(mfrow = c(1,1))
...
barplot(res, main='식사시간에 따른 팁 비율 분포',
